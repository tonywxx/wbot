...
- **Leverage realized (2026-08-09):** once callers take `&MarketRouter`, the A/US backtest duplication collapsed as predicted. `generate_reports` + `generate_reports_us` in `backtest_cli.rs` are now one private `generate_reports_for(out_dir, watchlist, market_label, warn_on_empty)` with two one-line wrappers; the daily-vs-intraday pick was extracted as `select_series(timeframe, code, klines, intraday) -> Option<Vec<Candle>>` in `backtest.rs` (pure, unit-tested). 28 lib tests pass.
